...
locals {
  monitors = [
    {
      name = "turning_step_trigger_1"
      index = "turning-index"
      query = jsonencode({
        "size": 0,
        "query": {"term": {"status": "alarm"}}
      })
    },
    {
      name = "turning_step_trigger_2"
      index = "turning-index"
      query = jsonencode({
        "size": 0,
        "query": {"range": {"vibration": {"gt": 5}}}
      })
    },
    {
      name = "turning_step_trigger_3"
      index = "turning-index"
      query = jsonencode({
        "size": 0,
        "query": {"range": {"temperature": {"gt": 90}}}
      })
    }
  ]
}
