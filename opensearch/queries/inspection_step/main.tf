...
locals {
  # Placeholder queries — replace with actual DSL matching your triggers
  monitors = [
    {
      name = "inspection_step_trigger_orange"
      index = "inspection-index"
      query = jsonencode({
        "size": 0,
        "query": {"term": {"severity": "orange"}}
      })
    },
    {
      name = "inspection_step_trigger_red"
      index = "inspection-index"
      query = jsonencode({
        "size": 0,
        "query": {"term": {"severity": "red"}}
      })
    },
    {
      name = "inspection_step_trigger_yellow"
      index = "inspection-index"
      query = jsonencode({
        "size": 0,
        "query": {"term": {"severity": "yellow"}}
      })
    }
  ]
}
