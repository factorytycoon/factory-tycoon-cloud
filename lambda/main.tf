...
resource "aws_lambda_function" "iot_to_cache" {
  ...
  environment {
    variables = {
      ...
      CACHE_TO_OPENSEARCH_FUNCTION_NAME = aws_lambda_function.cache_to_opensearch.function_name
    }
  }
...
# EventBridge Rule: schedule to trigger cache-to-mongodb
resource "aws_cloudwatch_event_rule" "every_2_minutes" {
  name                = "${var.project}-every-2m"
