...
for (def hit : ctx.results[0].hits.hits) {
    ...
    // 1. 유해 가스(voc) 점수 산정 (가중치 높음)
    // Red: 500 초과 (10점)
    if (voc > 500) { 
        score_voc = 10; 
    }
    // Orange: 150 ~ 499 (6점) - Score표 기준
    ...
    // 2. 분사 압력(pres) 점수 산정
    // Red: 1.0 미만 or 7.0 초과 (10점)
    if (pres < 1.0 || pres > 7.0) { 
        score_pres = 10; 
    }
    // Orange: 3.5 미만 or 4.8 초과 (3점) - Red 구간 제외
    else if (pres < 3.5 || pres > 4.8) { 
        score_pres = 3; 
    }
    // Yellow: 3.5~3.7 or 4.3~4.8 (2점)
    ...
    // Orange: 38 미만 or 55 초과 (3점) - Red 구간 제외
    else if (temp < 38 || temp > 55) { 
        score_temp = 3; 
    }
    // Yellow: 38~41 or 49~55 (2점)
