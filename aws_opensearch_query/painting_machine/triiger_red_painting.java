...
for (def hit : ctx.results[0].hits.hits) {
    ...
    // sensors 배열에서 도색 공정 센서 값 추출 (pres, voc, temp)
    ...
    // 1. 분사 압력(pres) 점수 산정
    // Red: 1.0 미만 or 7.0 초과
    if (pres < 1.0 || pres > 7.0) { 
        score_pres = 10; 
    }
    // Orange: 3.5 미만 or 4.8 초과 (Red 제외)
    else if (pres < 3.5 || pres > 4.8) { 
        score_pres = 3; 
    }
    // Yellow: 3.5~3.7 or 4.3~4.8
    ...
    // Orange: 150 ~ 499
        ...
        score_voc = 6; // Score표 기준 voc는 Orange가 6점
    }
    // Yellow: 50 ~ 149
    else if (voc >= 50 && voc <= 149) { 
        score_voc = 3; // Score표 기준 voc는 Yellow가 3점
    }
    ...
    // Orange: 38 미만 or 55 초과 (Red 제외)
    else if (temp < 38 || temp > 55) { 
        score_temp = 3; 
    }
    // Yellow: 38~41 or 49~55 (Green: 42~48 제외 구간)
